...
Hello, today I want to write about Linear Regression. It is a It is basically
...
accuracy of model.  Regarding the algorithm, we have a set of inputs
...
$$ X_{n+1 \space \times d} = \begin{bmatrix} (x^{(0)})^{T} \\ (x^{(1)})^{T}
...
$$ Y_{n+1 \space \times 1} = \begin{bmatrix} y^{(0)} \\ y^{(1)} \\ \vdots
\\ y^{(n)} \\ \end{bmatrix} $$

Our goal is to create a hypothesis function, $h$, that maps a new input
...
factors and constant by vector of $\theta$. As there are $d$ features, scaling
...
* Gradient Descent
  ...
  define an loss function $J(\theta)$, that is an indicator of the
  degree of error present in our model. We define this function in such a way that
  it captures inability of the system. The lower the value of
  ...
  square in $J(\theta)$, the positive and negative difference may cancel out, the error
  may be reduced, but the inaccuracies would still remain.

  Now our goal is to minimize the value of this function for our model.
  Since this is a minimization problem, we can use
  calculus to make our work easier. Remember that our goal is to adjust
  the parameters, $\theta$ to attain the values for which the loss function would be minimum. So, we take the
...
* Normal Equations
  ...
  be wondering /"What's a column space?/". That's why we need to understand some basic terminologies first.
...
*** Projection
    ...
    #+BEGIN_EXPORT html
    <figure>
    <img src="/images/posts/linear_regression/projection_vector.png">
    ...
    onto a columnspace means finding a linear combination of the columns which is closest to the vector
    ...
    #+BEGIN_EXPORT html
    <figure>
    <img src="/images/posts/linear_regression/projection_subspace.png">
    <figcaption>Projection of a vector onto a subspace</figcaption>
    &nbsp;
    </figure>
    #+END_EXPORT
